...
model222=load_model("model111.h5")
...
def upload11_file():
    ...
    if request.method == 'GET':
        ...
        try:
            ...
            full_name = os.path.join(UPLOAD_FOLDER, file.filename)
            file.save(full_name)
            indices = {0: 'Normal', 1: 'Pneumonia Detected', 2: 'Invasive carcinomar', 3: 'Normal'}
...
def result():
    ...
    if(int(result)==1):
        prediction='Oops ! Suffering'
    else:
        prediction='Great ! You are Healthy' 
    return(render_template("result.html", prediction=prediction))
